...
export const SaasPortalPage: React.FC<{ setActiveTab?: (tab: string) => void }> = ({ setActiveTab }) => {
  ...
  const handleSidebarClick = (id: SidebarView, tab?: string) => {
    setActiveView(id);
    if (tab && setActiveTab) {
      setActiveTab(tab);
    }
  ...
  return (
    <div className="min-h-screen bg-[#070b14] text-slate-100 font-sans selection:bg-cyan-500 selection:text-slate-950 flex -mx-4 sm:-mx-6 lg:-mx-8 -my-6 sm:-my-8">
      {/* Sidebar */}
      <aside
        className={`w-64 bg-[#0a0f1d] border-r border-slate-800/80 flex flex-col justify-between p-5 space-y-6 ${
          sidebarOpen ? 'block' : 'hidden lg:block'
        }`}
      >
        <div className="space-y-6">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-cyan-500 via-blue-600 to-indigo-600 flex items-center justify-center text-slate-950 font-bold shadow-lg shadow-cyan-500/20">
            ...
            <div>
              ...
              <span className="block text-[9px] text-slate-400 font-medium">Enterprise SaaS v3.0</span>
            </div>
          </div>
          ...
          <nav className="space-y-1 text-xs font-semibold">
            {[
              ...
              { id: 'defense' as SidebarView, label: 'MAP Defense Stream', icon: ShieldCheck, tab: 'brand-dashboard' },
              { id: 'sourcing' as SidebarView, label: 'Wholesale Deal Triage', icon: TrendingUp, tab: 'seller-dashboard' },
              { id: 'logistics' as SidebarView, label: '3PL Logistics Hubs', icon: Truck, tab: 'settings' },
              { id: 'integrations' as SidebarView, label: 'Amazon SP-API & LWA', icon: Store, tab: 'settings' },
              { id: 'security' as SidebarView, label: 'PostgreSQL RLS Sandboxing', icon: Database, tab: 'register' },
              { id: 'billing' as SidebarView, label: 'Subscription & Billing', icon: CreditCard, tab: 'settings' }
            ].map(item => {
              ...
              return (
                <button
                  key={item.id}
                  onClick={() => handleSidebarClick(item.id, item.tab)}
        ...
        <div className="p-3 border-t border-slate-800/80 bg-slate-950/50 rounded-xl space-y-2 text-xs">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="h-7 w-7 rounded-full bg-cyan-500/20 border border-cyan-500/40 flex items-center justify-center text-cyan-400 font-bold text-xs">
                {user?.name ? user.name.charAt(0).toUpperCase() : 'A'}
              </div>
              <div className="text-left text-xs truncate max-w-[100px]">
                <p className="font-semibold text-white truncate">{user?.name || 'Admin User'}</p>
                <p className="text-[9px] text-slate-400 font-mono capitalize">{user?.role || 'Admin'}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col p-4 sm:p-6 space-y-6 overflow-y-auto">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div>
            <div className="flex items-center gap-2 text-xs text-cyan-400 font-semibold uppercase tracking-wider mb-0.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {/* Card 1: GMV */}
          <div className="md:col-span-2 rounded-3xl p-6 bg-gradient-to-br from-slate-900/90 via-slate-900/60 to-slate-950 border border-slate-800/90 shadow-2xl relative overflow-hidden flex flex-col justify-between space-y-6">
          ...
          <div className="md:col-span-2 lg:col-span-3 rounded-3xl p-6 bg-slate-900/80 border border-slate-800/90 shadow-2xl space-y-6">
