/**
 * PostgreSQL Database Client & Queries for DistributionBridge
 * Compatible with Cloudflare Hyperdrive and standard PostgreSQL connections via postgres-js
 */
...
/**
 * Initializes or returns the cached PostgreSQL connection instance
 * @param {object} env - Cloudflare Worker environment bindings
 */
export function getDb(env) {
  // Use Hyperdrive connection string if bound, otherwise fallback to DATABASE_URL secret
  const connectionString = env.HYPERDRIVE?.connectionString || env.DATABASE_URL;

  if (!connectionString) {
    throw new Error(
      'Database connection string not configured. Please bind HYPERDRIVE or set DATABASE_URL secret via wrangler secret put DATABASE_URL.'
    );
  }

  // If connection string changed or not initialized, create a new instance
...
/**
 * Upserts Amazon Selling Partner OAuth tokens and credentials
 */
export async function upsertSellerCredentials(env, {
  sellingPartnerId,
  userId = null,
  accountName = null,
  refreshToken,
 ...
 */
export async function getSellerCredentials(env, sellingPartnerId) {
  const sql = getDb(env);
...
/**
 * Updates the cached access token and expiration for an existing seller
 */
export async function updateSellerAccessToken(env, sellingPartnerId, accessToken, expiresIn) {
  const sql = getDb(env);
